		本文件为CDBTune搭建训练model的引导文件。由于部分数据是腾讯公司的专利，所以在这里进行了去除，相应地方笔者会标注。
...
15. 将CDBTune1的/home/cheng/AutoTuner/environment/mysql.py中的TEMP_FILES和PROJECT_DIR变量中的用户名改为”cheng”（服务器的用户名）。

16. 将CDBTune1的/home/cheng/AutoTuner/scripts/run_sysbench.sh中的script_path中的用户名修改为”cheng”（服务器的用户名）。

17. 需要自行设置environment模块下knobs.py文件中各个knob的默认值。

18. 需要自行设置tuner模块下tuner_configs.py中的一些超参数的值。

19. 运行CDBTune1的tuner/train.py文件，开始训练model。
